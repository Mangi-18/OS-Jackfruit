...
nt child_fn(void *arg)
{
    ...
    // 2. Set hostname
    if (sethostname(req->container_id, strlen(req->container_id)) != 0) {
        perror("sethostname failed");
    }

    printf("Inside container (child process)\n");
	...
	setpriority(PRIO_PROCESS,0,req->nice_values);	
    // 5. Execute shell
...
static int cmd_run(int argc, char *argv[])
{
   ...
   printf("Running container %s with command: %s\n", req.container_id, req.command);
